...
let elab_ps env ps rettyp =
  let ctx =
    { ctx_return_typ = rettyp
    ; ctx_labels = StringSet.empty
    ; ctx_break = true
    ; ctx_continue = true
    ; ctx_in_switch = false
    ; ctx_vararg = false
    ; ctx_nonstatic_inline = false
    } in
    match ps with
    | PSTMT stmt -> (PSSTMT (fst (elab_stmt env ctx stmt)), env)
    | PLBRACE -> (PSLBRACE, env)
    | PRBRACE -> (PSRBRACE, env)
    | PIF (exp, loc) -> (PSIF (fst (elab_expr ctx loc env exp)), env)
    | PELSE -> (PSELSE, env)
    | PCOM com -> (PSCOM com, env)
    | PWHILE (exp, loc) -> (PSWHILE (fst (elab_expr ctx loc env exp)), env)
    | PFOR (exp1, exp2, exp3, loc) ->
        let exp1 = fst (elab_opt_expr ctx loc env exp1) in
        let exp2 = fst (elab_opt_expr ctx loc env exp2) in
        let exp3 = fst (elab_opt_expr ctx loc env exp3) in
        (PSFOR (exp1, exp2, exp3), env)
    | PDECL def -> 
        let (decls, new_env) = elab_definition false true false env def in
        (PSDECL decls, new_env)

let env = ref (Env.initial ())

let curr_rettyp = ref (TVoid [])

let not_gcompdecl = function
  | { gdesc = C.Gcompositedecl _; gloc = _ } -> false
  | _ -> true

let elab_ps_file partial_stmt =
  try
    let env1 = C2C.translEnv !env !top_declarations in
    let comps = C2C.convertCompositedefs env1 [] !top_declarations in
    match Ctypes.build_composite_env (List.rev comps) with
    | Errors.Error msg ->
        C2C.fatal_error "incorrect struct or union definition: %s" (C2C.string_of_errmsg msg)
    | Errors.OK cenv ->
        C2C.comp_env := cenv;
        let (ps, env2) = elab_ps env1 partial_stmt !curr_rettyp in
        begin match ps with
        | PSDECL [] -> curr_rettyp := TVoid []
        | PSDECL [(_, {C.name = "__return"; _}, typ, _)] -> curr_rettyp := typ
        | _ -> ()
        end;
        env := env2;
        top_declarations := List.filter not_gcompdecl !top_declarations;
        ps
  with
    Env.Error msg ->
      C2C.fatal_error "%s" (Env.error_message msg)
