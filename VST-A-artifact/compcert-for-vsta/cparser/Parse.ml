...
let parse_ps_string name text =
  let log_fuel = Camlcoq.Nat.of_int 50 in
  match
    Parser.translation_ps_file log_fuel (Lexer.tokens_stream_ps name text)
  with
    | Parser.MenhirLibParser.Inter.Parsed_pr (ast, _) -> ast
    | _ -> Diagnostics.fatal_error Diagnostics.no_loc "internal error while parsing"
