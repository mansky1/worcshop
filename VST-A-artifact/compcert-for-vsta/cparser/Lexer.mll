...
let charlist_to_string_rev acc =
  let len = List.length acc in
  let s = Bytes.create len in
    List.iteri (fun i c -> Bytes.set s (len - 1 - i) c) acc;
    Bytes.to_string s
}
...
rule initial = parse
  ...
  | "/*@"                         { let loc = currentLoc lexbuf in
                                    let a_str = multiline_assertion [] lexbuf in
                                      COMMENT (a_str, loc) }
  | "/*"                          { multiline_comment lexbuf; initial lexbuf }
  | "//@"                         { let loc = currentLoc lexbuf in
                                    let a_str = singleline_assertion [] lexbuf in
                                      COMMENT (a_str, loc) }
...
and multiline_assertion acc = parse
  | "*/"   { charlist_to_string_rev acc }
  | eof    { error lexbuf "unterminated comment"; "" }
  | '\n'   { new_line lexbuf; multiline_assertion ('\n' :: acc) lexbuf }
  | _ as c { multiline_assertion (c :: acc) lexbuf }
...
and singleline_assertion acc = parse
  | '\n'   { new_line lexbuf; charlist_to_string_rev acc }
  | eof    { charlist_to_string_rev acc }
  | _ as c { singleline_assertion (c :: acc) lexbuf }

{
  ...
  let invoke_pre_parser_with transl filename text lexer buffer =
    ...
    let checkpoint = transl lexbuf.lex_curr_p
  ...
  let invoke_pre_parser =
    invoke_pre_parser_with Pre_parser.Incremental.translation_unit_file

  (* [tokens_stream filename text] runs the pre_parser and produces a stream
     of (appropriately classified) tokens. *)

  let tokens_stream_with invoke_parser filename text : buffer =
    let tokens = Queue.create () in
    let buffer = ref ErrorReports.Zero in
    invoke_parser filename text (lexer tokens buffer) buffer;
  ...
  let tokens_stream = tokens_stream_with invoke_pre_parser

  let invoke_ps_pre_parser =
    invoke_pre_parser_with Pre_parser.Incremental.translation_ps_file

  let tokens_stream_ps =
    tokens_stream_with invoke_ps_pre_parser
