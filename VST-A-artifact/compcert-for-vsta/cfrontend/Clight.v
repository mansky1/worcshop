...
Inductive partial_statement_with {A : Type} : Type :=
  | PSskip : partial_statement_with                   (**r do nothing *)
  | PSassign : expr -> expr -> partial_statement_with (**r assignment [lvalue = rvalue] *)
  | PSset : ident -> expr -> partial_statement_with
  | PScall: option ident -> expr -> list expr -> partial_statement_with (**r function call *)
  | PSbuiltin: option ident -> external_function -> typelist -> list expr -> partial_statement_with (**r builtin invocation *)
  | PSbreak : partial_statement_with                       (**r [break] statement *)
  | PScontinue : partial_statement_with                    (**r [continue] statement *)
  | PSreturn : option expr -> partial_statement_with       (**r [return] statement *)
  | PSwhile : expr -> partial_statement_with               (**r while(b) **)
  | PSfor : option partial_statement_with -> option expr -> option partial_statement_with -> partial_statement_with
  | PSif : expr -> partial_statement_with                  (**r if (b) **)
  | PSelse : partial_statement_with
  | PSlbrace : partial_statement_with
  | PSrbrace : partial_statement_with
  | PSassert : A -> partial_statement_with
  | PSdecl : list (ident * type) -> partial_statement_with
  | PSfake : statement -> partial_statement_with
  | PSsequence : partial_statement_with -> partial_statement_with -> partial_statement_with.

Arguments partial_statement_with: clear implicits.

Definition partial_statement := partial_statement_with comment.
                    
Fixpoint stmt2ps (s: statement) : partial_statement :=
  match s with
  | Sassign e1 e2 => PSassign e1 e2
  | Sset temp v => PSset temp v
  | Scall a b c => PScall a b c
  | Sreturn e_opt => PSreturn e_opt
  | Sskip => PSskip 
  | Ssequence s1 s2 => PSsequence (stmt2ps s1) (stmt2ps s2)
  | Sbreak => PSbreak
