...
let convertOptExpr env = function
  | None -> None
  | Some e -> Some (convertExpr env e)

let convertPS env = function
  | PSLBRACE -> Csyntax.PSlbrace
  | PSRBRACE -> Csyntax.PSrbrace
  | PSSTMT s -> Csyntax.PSstmt (convertStmt env s)
  | PSIF e -> Csyntax.PSif (convertExpr env e)
  | PSELSE -> Csyntax.PSelse 
  | PSCOM c -> Csyntax.PScom c
  | PSWHILE e -> Csyntax.PSwhile (convertExpr env e)
  | PSFOR (e1, e2, e3) ->
      let e1 = convertOptExpr env e1 in
      let e2 = convertOptExpr env e2 in
      let e3 = convertOptExpr env e3 in
