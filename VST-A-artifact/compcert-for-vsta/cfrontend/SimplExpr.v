...
Definition transl_ps_mon (ps : Csyntax.partial_statement) : mon partial_statement :=
  match ps with
  | Csyntax.PSlbrace => ret PSlbrace
  | Csyntax.PSrbrace => ret PSrbrace
  | Csyntax.PSstmt s => 
      do tbody <- transl_stmt s;
      match simpl_stmt VSet.empty tbody with
      | Errors.Error msg1 => error msg1
      | Errors.OK ps => ret (stmt2ps ps)
      end
  | Csyntax.PSif e =>
      do (_, e') <- transl_expression e;
      ret (PSif (simpl_expr VSet.empty e'))
  | Csyntax.PSelse => ret PSelse
  | Csyntax.PScom c => ret (PSassert c)
  | Csyntax.PSwhile e =>
      do (_, e') <- transl_expression e;
      ret (PSwhile (simpl_expr VSet.empty e'))
  | Csyntax.PSfor e1 e2 e3 =>
      do st1 <-
        match e1 with
        | None => ret None
        | Some e1 => 
            do s <- transl_expr_stmt e1;
            ret (Some (stmt2ps s))
        end;
      do e2 <-
        match e2 with
      ...
      do st3 <-
        ...
        | None => ret None
        | Some e3 => 
            do s <- transl_expr_stmt e3;
            ret (Some (stmt2ps s))
        end;
      ret (PSfor st1 e2 st3)
  | Csyntax.PSdecl vars => ret (PSdecl vars)
  end.

Definition transl_ps (ps : Csyntax.partial_statement) : res (partial_statement * generator) :=
  match transl_ps_mon ps (initial_generator tt) with
  | Err msg => Error msg
  | Res ps g i => OK (ps, g)
  end.
...
(* Definition transl_ps (ps : Csyntax.partial_statement) : res (partial_statement * generator) :=
  match ps with
  | Csyntax.PSlbrace => OK (PSlbrace, initial_generator tt)
  | Csyntax.PSrbrace => OK (PSrbrace, initial_generator tt)
  | Csyntax.PSstmt s => 
      match transl_stmt s (initial_generator tt) with
      | Err msg =>
          Error msg
      | Res tbody g i =>
          match simpl_stmt VSet.empty tbody with
          | Errors.Error msg1 => Error msg1
          | Errors.OK ps => OK (stmt2ps ps, g)
          end
      end
  | Csyntax.PSif e =>
      match transl_expression e (initial_generator tt) with
      | Err msg =>
          Error msg 
      | Res (s, e') g i =>
          OK (PSif (simpl_expr VSet.empty e'), g)
      end
  | Csyntax.PSelse => OK (PSelse, initial_generator tt)
  | Csyntax.PScom c => OK (PSassert c, initial_generator tt)
  | Csyntax.PSwhile e => 
      match transl_expression e (initial_generator tt) with
      | Err msg =>
          Error msg 
      | Res (s, e') g i =>
          OK (PSwhile (simpl_expr VSet.empty e'), g)
      end
  | Csyntax.PSfor e1 e2 e3 =>

  | Csyntax.PSdecl vars => OK (PSdecl vars, initial_generator tt)
  end. *)
