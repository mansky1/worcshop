...
Lemma expr_rep'_expr_rep:
  forall (p: val) (e: expr),
    expr_rep' e p |-- expr_rep p.
Proof.
...
Ltac destruct_expr :=
  ...
  | |- context [expr_rep _] =>
    ...
    try lia;
    repeat sep_apply expr_rep'_expr_rep
  end.
...
Lemma cmd_rep'_cmd_rep:
  forall (p: val) (c: cmd),
    cmd_rep' c p |-- cmd_rep p.
Proof.
...
Lemma cont_list_rep'_cont_list_rep:
  forall (p: val) (l: list cmd),
    cont_list_rep' l p
    |-- cont_list_rep p.
Proof.
...
Ltac destruct_prog :=
  match goal with
  | |- context [prog_rep _] =>
    unfold prog_rep at 1;
    let prog := fresh "prog" in
    Intros prog;
    let foc := fresh "foc" in
    let ectx := fresh "ectx" in
    let cp := fresh "cp" in
    let link := fresh "link" in
    destruct prog; simpl prog_rep';
      [ Intros foc ectx
      | Intros ectx cp link ];
    forward;
    repeat match goal with
    ...
    repeat sep_apply cont_list_rep'_cont_list_rep;
    repeat sep_apply cmd_rep'_cmd_rep;
    repeat sep_apply expr_rep'_expr_rep
  end.
...
Proof.
  intros. unfold expr_rep.
  Exists (E_Const v). simpl expr_rep'.
  entailer!.
Qed.
...
Proof.
  intros. unfold expr_rep.
  Exists (E_Var s). simpl expr_rep'.
  entailer!.
Qed.

Lemma binop_expr_rep:
  forall (p: val) (op: binop) (arg1 arg2: val),
    store_expr p 2 0 nullval (enum_of_binop op) arg1 arg2 *
    expr_rep arg1 *
    expr_rep arg2
    |-- expr_rep p.
Proof.
  intros. unfold expr_rep. Intros e2 e1.
  Exists (E_Binop op e1 e2). simpl expr_rep'.
  Exists arg1 arg2.
  entailer!.
Qed.

Lemma deref_expr_rep:
  forall (p: val) (arg: val),
    store_expr p 3 0 nullval 0 arg nullval *
    expr_rep arg
    |-- expr_rep p.
Proof.
  intros. unfold expr_rep. Intros e.
  Exists (E_Deref e). simpl expr_rep'.
  Exists arg.
  entailer!.
Qed.

Lemma malloc_expr_rep:
  forall (p: val),
    store_expr p 4 0 nullval 0 nullval nullval
    |-- expr_rep p.
Proof.
  intros. unfold expr_rep.
  Exists E_Malloc. simpl expr_rep'.
  entailer!.
Qed.
...
Proof.
  intros. unfold cmd_rep.
  Exists (C_Decl s). simpl cmd_rep'.
  entailer!.
Qed.

Lemma asgn_cmd_rep:
  forall (p: val) (arg1 arg2: val),
    store_cmd p 1 nullval arg1 arg2 nullval nullval *
    expr_rep arg1 *
    expr_rep arg2
    |-- cmd_rep p.
Proof.
  intros. unfold expr_rep, cmd_rep.
  Intros e2 e1.
  Exists (C_Asgn e1 e2). simpl cmd_rep'.
  Exists arg1 arg2. entailer!.
Qed.

Lemma seq_cmd_rep:
  forall (p: val) (sub1 sub2: val),
    store_cmd p 2 nullval nullval nullval sub1 sub2 *
    cmd_rep sub1 *
    cmd_rep sub2
    |-- cmd_rep p.
Proof.
  intros. unfold cmd_rep.
  Intros c2 c1.
  Exists (C_Seq c1 c2). simpl cmd_rep'.
  Exists sub1 sub2. entailer!.
Qed.

Lemma if_cmd_rep:
  forall (p: val) (arg sub1 sub2: val),
    store_cmd p 3 nullval arg nullval sub1 sub2 *
    expr_rep arg *
    cmd_rep sub1 *
    cmd_rep sub2
    |-- cmd_rep p.
Proof.
  intros. unfold expr_rep, cmd_rep.
  Intros subf subt cond.
  Exists (C_If cond subt subf). simpl cmd_rep'.
  Exists arg sub1 sub2. entailer!.
Qed.

Lemma while_cmd_rep:
  forall (p: val) (arg sub: val),
    store_cmd p 4 nullval arg nullval sub nullval *
    expr_rep arg *
    cmd_rep sub
    |-- cmd_rep p.
Proof.
  intros. unfold expr_rep, cmd_rep.
  Intros body cond.
  Exists (C_While cond body). simpl cmd_rep'.
  Exists arg sub. entailer!.
Qed.

Lemma null_cont_list_rep:
  emp |-- cont_list_rep nullval.
Proof.
  intros. unfold cont_list_rep.
  Exists (@nil cmd).
  simpl cont_list_rep'. entailer!.
Qed.

Lemma store_cont_list_rep :
  forall (c link v : val),
...
Lemma foc_prog_rep_or_end:
  forall (p: val) (foc ectx: val),
    store_res_prog p foc ectx *
    cmd_rep foc *
    cont_list_rep ectx
    |-- prog_rep_or_end p.
Proof.
  intros. unfold prog_rep_or_end, cmd_rep, cont_list_rep.
  Intros l c. Exists (Some (P_Foc c l)).
  simpl prog_rep_or_end'. Exists foc ectx.
  entailer!.
Qed.

Lemma ectx_prog_rep_or_end:
  forall (p: val) (ectx: val),
    store_res_prog p nullval ectx *
    cont_list_rep ectx
    |-- prog_rep_or_end p.
Proof.
  intros. unfold prog_rep_or_end, cont_list_rep.
  Intros l. destruct l.
  - simpl cont_list_rep'. Intros; subst.
    Exists (@None res_prog).
    simpl prog_rep_or_end'. cancel.
  - Exists (Some (P_Ectx c l)).
    unfold prog_rep_or_end', prog_rep'.
    Exists ectx.
    entailer!.
Qed.
