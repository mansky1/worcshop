...
Tactic Notation "list_intros" := sep_apply listrep_null; Intros.

Tactic Notation "list_intros" simple_intropattern(a)
    simple_intropattern(b) simple_intropattern(c) :=
  match goal with
  | H: isptr ?x |- context [listrep ?l ?x] =>
      sep_apply (listrep_isptr l x); Intros a b c
  | H: ?x <> nullval |- context [listrep ?l ?x] =>
      sep_apply (listrep_is_not_null l x); Intros a b c
  end.
