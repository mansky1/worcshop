...
Tactic Notation "list_intros" :=
  match goal with
  | |- context [listrep_pre _ nullval _] => sep_apply listrep_pre_null
  | |- context [lseg_pre _ nullval _ _ _] => sep_apply lseg_pre_null
  end;
  Intros.

Tactic Notation "list_intros" simple_intropattern(a)
    simple_intropattern(b) simple_intropattern(c) :=
  match goal with
  | H: isptr ?x |- context [listrep_pre ?l ?x _] =>
      sep_apply (listrep_pre_isptr l x)
  | H: ?x <> nullval |- context [listrep_pre ?l ?x _] =>
      sep_apply (listrep_pre_is_not_null l x)
  end;
  Intros a b c.
